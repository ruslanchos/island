...
import ru.javarush.island.khmelov.entity.map.Service;
...
public abstract class Organism implements Reproducible, Cloneable {
    ...
    protected boolean safeDie(Service target) {
        target.getLock().lock();
        try {
            return target
                    .getResidents()
    ...
    protected boolean safeChangeWeight(Service currentService, int percent) {
        currentService.getLock().lock();
        try {
            ...
            currentService.getLock().unlock();
        }
    }


    protected boolean safeMove(Service source, Service destination) {
    ...
    protected boolean safeAddTo(Service Service) {
        Service.getLock().lock();
        try {
            Organisms organisms = Service
                    .getResidents()
                    .get(getType());
            int maxCount = getLimit().getMaxCountInService();
    ...
    protected boolean safePollFrom(Service Service) {
        Service.getLock().lock();
        try {
            Residents residents = Service.getResidents();
    ...
    protected boolean safeFindFood(Service currentService) {
        currentService.getLock().lock();
        ...
        try {
            ...
            if (!(needFood <= 0)) {
                ...
                while (needFood > 0 && foodIterator.hasNext()) {
                    ...
                    Residents residents = currentService.getResidents();
